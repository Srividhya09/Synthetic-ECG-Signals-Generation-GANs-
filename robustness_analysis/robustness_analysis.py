...
# st.markdown('<div class="header-style">Robustness Evaluation of Synthetic ECG</div>', unsafe_allow_html=True)

# if st.button("Run Robustness Evaluation"):
#     st.markdown('<p style="color:white;">Running DTW-based robustness analysis...</p>', unsafe_allow_html=True)


#     # -----------------------------
#     # 1. Load Real ECG (Reference Signal)
#     # -----------------------------
#     with open("ECG_data.pkl", "rb") as f:
#         real_ecg = pickle.load(f)

#     # Pick one patient (first one)
#     patient_id = list(real_ecg.keys())[0]
#     label, full_real_signal = real_ecg[patient_id]
#     full_real_signal = np.asarray(full_real_signal, dtype=float).flatten()

#     # -----------------------------
#     # 2. Load Synthetic ECG (use first generated or uploaded signal)
#     # -----------------------------
#     try:
#         # Prefer generated signals if available
#         synthetic_signal = st.session_state.signals[0].squeeze()
#     except:
#         # If no generated signals, try uploaded file
#         if 'signals' in locals():
#             synthetic_signal = signals[0].squeeze()
#         else:
#             st.warning("No synthetic ECG signal available. Please generate or upload a signal first.")
#             st.stop()

#     SEGMENT_LENGTH = len(synthetic_signal)
#     real_signal = full_real_signal[:SEGMENT_LENGTH]

#     # -----------------------------
#     # 3. Noise Injection Function
#     # -----------------------------
...
#         noisy_signal = add_gaussian_noise(synthetic_signal, level)
#         distance, _ = fastdtw(real_signal, noisy_signal, dist=lambda x, y: abs(x - y))
#         dtw_scores.append(distance)
#         st.write(f"Noise Level: {level} | DTW Distance: {distance:.4f}")

#     # -----------------------------
#     # 5. Visualization
#     # -----------------------------
#     fig, ax = plt.subplots(figsize=(6, 4))
#     ax.plot(noise_levels, dtw_scores, marker='o', color='cyan')
#     ax.set_xlabel("Noise Level")
#     ax.set_ylabel("DTW Distance")
#     ax.set_title("Robustness Analysis of Synthetic ECG")
#     ax.grid(True)
#     st.pyplot(fig)
...
if st.button("Run Robustness Evaluation"):
    ...
    # -----------------------------
    # 2. Load default Synthetic ECG (first signal in .npy file)
    # -----------------------------
    try:
        synthetic_ecg = np.load("synthetic_ecg_signals.npy")
        synthetic_signal = np.asarray(synthetic_ecg[0], dtype=float).flatten()
    except Exception as e:
        st.error(f"Error loading default synthetic ECG: {e}")
        st.stop()
    ...
    for level in noise_levels:
        ...
        distance, _ = fastdtw(real_signal, noisy_signal, dist=lambda x, y: abs(x - y))
        dtw_scores.append(distance)
        st.write(f"<span style='color:white'>Noise Level: {level} | DTW Distance: {distance:.4f}</span>", unsafe_allow_html=True)
